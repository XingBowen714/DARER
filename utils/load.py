...
class DataHub(object):
    ...
    @classmethod
    def from_dir_addadj(cls, args):
        house = DataHub()

        house._data_collection["train"] = house._read_data(
            os.path.join(args.data_dir, "train.json"), True
        )
        house._data_collection["dev"] = house._read_data(
            os.path.join(args.data_dir, "dev.json"), True
        )
        house._data_collection["test"] = house._read_data(
            os.path.join(args.data_dir, "test.json"), True
        ) 
        return house
